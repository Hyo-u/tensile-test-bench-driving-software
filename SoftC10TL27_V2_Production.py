...
DOSSIER_CONFIG_ET_CONSIGNES = lecture_donnee("dossier_config_et_consignes-production.txt") + SEPARATEUR
...
def modification_des_PID(parent):
   ...
   def validation_des_nouveaux_PID():
      ...
      if type_de_materiau.get() == 0 :
         fichier_des_PID = DOSSIER_CONFIG_ET_CONSIGNES + "pid_mou.txt"
      else :
         fichier_des_PID = DOSSIER_CONFIG_ET_CONSIGNES + "pid_rigide.txt"
   ...
   def valeurs_actuelles_des_PID():
      ...
      if type_de_materiau.get() == 0 :
         with open(DOSSIER_CONFIG_ET_CONSIGNES + "pid_mou.txt", 'r') as fichier_PID :
            dic_PID = load (fichier_PID)
      else :
         with open(DOSSIER_CONFIG_ET_CONSIGNES + "pid_rigide.txt", 'r') as fichier_PID :
            dic_PID = load (fichier_PID)
   ...
   with open(DOSSIER_CONFIG_ET_CONSIGNES + "pid_personnalise.txt", 'r') as fichier_PID :
      dic_PID = load (fichier_PID)
#V
...
def configuration_initiale (init_titre, init_nom, 
   ...
   def validation_des_entrees() :
      nonlocal dic_PID
      if choix_PID.get() == 2 :
         with open(DOSSIER_CONFIG_ET_CONSIGNES + "pid_personalise.txt", 'w') as fichier_PID_perso :
...
###fonction de fenêtre graphique
   ...
   def enregistrer_et_quitter():
      ...
      Button(fenetre_de_sortie_du_programme,text='Enregistrer et quitter', command=quitter).grid(row=5,column=2,padx =10, pady =10)
   #V        
 ...
 #       Voir sensi_page() et reglage_des_coef_des_PID() dans les fonctions jetées.
   ...
   def choix_des_consignes_du_generateur():
      ...
      def surcouche_modification(indice_de_cette_consigne):
         ...
         #TODO Ensure indice_de_cette_consigne is a valid index
         ...
         if consigne_a_modifier is not None :
            ...
            if verrou_production == ON :
               ...
               # 5 : palier à cette valeur
               #TODO Faire en sorte que la valeur du palier soit egale a la valeur de consigne
               """
               condition_string = consignes_du_generateur[2]["condition"]
               match = re.search(r'\d+\.\d+', condition_string)
               if match:
                   numeric_value = float(match.group())
                   print("Extracted numeric value:", numeric_value)
               else:
                   print("No numeric value found in the condition string.")
               consignes_du_generateur[3]["value"] = match
                """
            return actualisation_des_boutons()
      #V
         ...
         if type_d_asservissement == ASSERVISSEMENT_EN_CHARGE :
            match consigne_a_changer["type"] :
               ...
               case "ramp" :
                  ...
                  #TODO
                  Label(fenetre_de_modification_d_une_consigne, text="Vitesse en tonnes/secondes :").grid(row=1,
                                                                                                          column=0,
                                                                                                          padx=5,
                                                                                                          pady=5)
                  Entry(fenetre_de_modification_d_une_consigne, width=5, textvariable=speed, validate="key",
                        validatecommand=(
                        fenetre_de_modification_d_une_consigne.register(_check_entree_vitesse_charge), '%P')).grid(
      ...
      def actualisation_des_boutons():
         ...
         #TODO
         if verrou_production == ON or OFF:#OFF
            ...
            if len(consignes_du_generateur) :
               ...
               for consigne_du_generateur in consignes_du_generateur :
                  ...
                  if verrou_production == OFF: #Avant rien
   ...
   while premieres_consignes_validees == False :
      ...
      if entrees[10] :
         ...
         if verrou_production == OFF :
            ...
            try :
               if type_d_asservissement == ASSERVISSEMENT_EN_CHARGE :
                  fichier_des_consignes_du_dernier_test = open(DOSSIER_CONFIG_ET_CONSIGNES + "consignes_du_test_precedent_production.json", 'r')
